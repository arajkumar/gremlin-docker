...
RUN git clone https://github.com/arajkumar/dynamodb-janusgraph-storage-backend.git --branch janusgraph-0.4  /opt/dynamodb/dynamodb-janusgraph-storage-backend/ &&\
...
RUN cd /opt/dynamodb/dynamodb-janusgraph-storage-backend/ &&\
    ...
    sed -i 's#JANUSGRAPH_VANILLA_SERVER_ZIP=.*#JANUSGRAPH_VANILLA_SERVER_ZIP=/opt/dynamodb/dynamodb-janusgraph-storage-backend/server/janusgraph-0.4.0-hadoop2.zip#' src/test/resources/install-gremlin-server.sh &&\
...
RUN mkdir -p ${M2_DIR}/root &&\
    ...
    curl -o ${M2_REPO}/org/slf4j/slf4j-api/1.7.21/slf4j-api-1.7.21.jar http://repo1.maven.org/maven2/org/slf4j/slf4j-api/1.7.21/slf4j-api-1.7.21.jar
